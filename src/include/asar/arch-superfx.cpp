...
static int64_t getnum_ck_sfx(const char* math)
{
...
static bool getreg(const char * par_sfx, int * reg, reg_t type)
{
	int ret;
	*reg=-1;
	if (type==reg_parr && *par_sfx++!='(') return false;
	if (type==reg_parr && to_lower(*par_sfx++)!='r') return false;
	if (type==reg_r && to_lower(*par_sfx++)!='r') return false;
	if (type==reg_hash && *par_sfx++!='#') return false;
	if (!is_digit(par_sfx[0])) return false;
	if (is_digit(par_sfx[1]))
	{
		if (par_sfx[0]!='1' || par_sfx[1]>'5') return false;
		ret=par_sfx[1]-'0'+10;
		par_sfx+=2;
	}
	else
...
{
#define is_sfx(test) (!stricmp(word[0], test))
	char * par_sfx= nullptr;
	if (word[1]) par_sfx= duplicate_string(word[1]);
	autoptr<char*> parptr=par_sfx;
	if(0);
	else if (assemblemapper(word, numwords)) return true;
	else if (numwords==1)
	{
#define op(from, to) if (is_sfx(from)) { write1(to); return true; }
#define op3d(from, to) if (is_sfx(from)) { write1(0x3D); write1(to); return true; }
#define op3e(from, to) if (is_sfx(from)) { write1(0x3E); write1(to); return true; }
#define op3f(from, to) if (is_sfx(from)) { write1(0x3F); write1(to); return true; }
		op("STOP", 0x00);
		op("NOP", 0x01);
		op("CACHE", 0x02);
		op("LSR", 0x03);
...
#undef op3f
	...
	else if (numwords==2)
	{
		string tmp=par_sfx;
		int numwordsinner;
		autoptr<char*> parcpy= duplicate_string(par_sfx);
		autoptr<char**> arg=qpsplit(parcpy, ",", &numwordsinner);
		bool ret=false;
#define ok() ret=true
#define op(op) if (is_sfx(op)) ok()
...
#define reg_range(min, max) ,range(min, reg, max)
			...
			if (getreg(par_sfx, &reg, reg_hash))
			{
			...
			if (getreg(par_sfx, &reg, reg_parr))
			{
...
#undef reg_range
			int byte=-1;
#define br(name, val) if (is_sfx(name)) byte=val;
			br("BRA", 0x05);
			br("BGE", 0x06);
			br("BLT", 0x07);
			br("BNE", 0x08);
...
#undef br
			...
			{
				ret=true;
				int len=getlen(par_sfx);
				unsigned int num=getnum_ck_sfx(par_sfx);
				if (len==1)
				{
				...
				{
					int pos=(int)getnum_ck_sfx(par_sfx)-((snespos&0xFFFFFF)+2);
...
#define w(val) ,write1((unsigned int)(val))
			...
			if (isreg1)
			{
				...
				if (arg[1][0]=='#')
				{
					unsigned int num=getnum_ck_sfx(arg[1]+1);
					num&=0xFFFF;
				...
				else if (arg[1][0]=='(')
				{
					...
					unsigned int num=getnum_ck_sfx(arg[1]);
