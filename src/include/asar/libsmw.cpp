...
int getpcfreespace(int size, bool isforcode, bool autoexpand, bool respectbankborders, bool align, unsigned char freespacebyte)
{
	...
	if (mapper==hirom)
	{
		if (isforcode)
		{
			for(int i = 0x8000; i < argmin(romlen, 0x400000); i += 0xFFFF){
				int space = trypcfreespace(i, argmin(i+0x7FFF, romlen), size, 0x7FFF, align?0xFFFF:0, freespacebyte);
	...
	if (mapper==exlorom)
	{
		...
		if (isforcode)
		{
			trypcfreespace(0, argmin(romlen, 0x200000), size, 0x7FFF, align?0x7FFF:0, freespacebyte);
		}
