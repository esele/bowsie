...
static bool bitmatch_spc700(const char * opnamein, string& opnameout, const char * str, string& math, int& bit)
{
...
{
#define is_spc700(test) (!stricmp(word[0], test))
#define is1_spc700(test) (!stricmp(word[0], test) && numwords==2)
#define par word[1]
	if(0);
	else if (arch==arch_spc700_inline && is1_spc700("org"))
	{
		unsigned int num=getnum(par);
		if (foundlabel) asar_throw_error(0, error_type_block, error_id_org_label_invalid);
		inline_org(num);
	}
	else if (arch==arch_spc700_inline && is1_spc700("arch"))
	{
		inline_leavearch();
		return false;
	}
	else if (arch==arch_spc700_inline && is1_spc700("skip"))
	{
		int num=snespos+(int)getnum(par);
		if (foundlabel) asar_throw_error(0, error_type_block, error_id_skip_label_invalid);
		inline_org(num);
	}
	else if (arch==arch_spc700_inline && is1_spc700("base"))
	{
		asar_throw_error(0, error_type_block, error_id_spc700_inline_no_base);
	}
	else if (arch==arch_spc700_inline && is1_spc700("startpos"))
	{
		inlinestartpos=(int)getnum_ck(par);
	}
	else if (numwords==1)
	{
#define op(name, val) else if (is_spc700(name)) do { write1(val); } while(0)
		if(0);
		op("nop", 0x00);
		op("brk", 0x0F);
		op("clrp", 0x20);
...
#define ismatch_spc700(left, right) (matchandwrite(arg[0], left, right, math))
...
#define match_spc700(left, right) if (ismatch_spc700(left, right))
			eq("a")
			{
				op0("asl", 0x1C);
				op0("das", 0xBE);
			...
			if (isop("jmp") && ismatch_spc700("(", "+x)")) w2(0x1F);
			match_spc700("", "+x")
			{
				op1("asl", 0x1B);
				op1("dec", 0x9B);
			...
			if (bitmatch_spc700(word[0], op, arg[0], math, bits))
			{
...
#define w1r(opcode, math1, math2) do { int len=getlen(math2); int num=(int)getnum_ck(math2); int pos=(len==1)?num:num-(snespos+3); \
			...
			int bits;
#define isop(test) (!stricmp(op, test))
			if (!stricmp(arg[0], "c") && bitmatch_spc700(word[0], op, arg[1], math, bits))
			{
				if (assinglebitwithc(op, math, bits)) return true;
			}
			if (bitmatch_spc700(word[0], op, arg[0], s1, bits))
			{
...
#undef isop
			...
			if (is_spc700("or") || is_spc700("and") || is_spc700("eor") || is_spc700("cmp") || is_spc700("adc") || is_spc700("sbc"))
			{
				int offset = 0;
				if (is_spc700("or" )) offset=0x00;
				if (is_spc700("and")) offset=0x20;
				if (is_spc700("eor")) offset=0x40;
				if (is_spc700("cmp")) offset=0x60;
				if (is_spc700("adc")) offset=0x80;
				if (is_spc700("sbc")) offset=0xA0;
			...
			vc("","", "a")
			{
				if (is_spc700("tset")) w2(0x0E, s1);
				if (is_spc700("tclr")) w2(0x4E, s1);
			}
			if (is_spc700("div") && iscc("ya", "x")) w0(0x9E);
			cv("ya", "","")
			{
				if (is_spc700("cmpw")) w1(0x5A, s2);
				if (is_spc700("addw")) w1(0x7A, s2);
				if (is_spc700("subw")) w1(0x9A, s2);
				if (is_spc700("movw")) w1(0xBA, s2);
			}
			if (is_spc700("movw") && isvc("","", "ya")) w1(0xDA, s1);
			if (is_spc700("cbne") && isvv("","+x", "","")) w1r(0xDE, s1, s2);
			if (is_spc700("dbnz") && iscv("y", "","")) wr(0xFE, s2);
			vv("","", "","")
			{
				if (is_spc700("dbnz")) w1r(0x6E, s1, s2);
				if (is_spc700("cbne")) w1r(0x2E, s1, s2);
			}
#undef iscc
#undef iscv
#undef isvc
