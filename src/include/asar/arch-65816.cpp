...
{
#define is_65816(test) (!stricmpwithupper(word[0], test))
//#define par word[1]
	string par;
...
#define match_65816(left, right) (word[1] && stribegin(par, left) && striend(par, right))
...
#define end_65816() return false
#define withlen(n) outlen=n; if(fake) return true
#define as0(    op, byte) if (is_65816(op)          ) { withlen(0); write1((unsigned int)byte);              return true; }
#define as1(    op, byte) if (is_65816(op) && len==1) { withlen(1); write1((unsigned int)byte); write1(num); return true; }
#define as2(    op, byte) if (is_65816(op) && (len==2 || (!explicitlen && len<2))) { withlen(2); write1((unsigned int)byte); write2(num); return true; } \
													/*if (is_65816(op) && len==3 && emulate) { write1(byte); write2(num); return true; }*/
#define as3(    op, byte) if (is_65816(op) && (len==3 || (!explicitlen && len<3))) { withlen(3); write1((unsigned int)byte); write3(num); return true; }
//#define as23(   op, byte) if (is_65816(op) && (len==2 || len==3)) { write1(byte); write2(num); return true; }
#define as32(   op, byte) if (is_65816(op) && ((len<3 && !explicitlen) || len==3)) { withlen(3); write1((unsigned int)byte); write3(num); return true; }
#define as_a(   op, byte) if (is_65816(op)) { if(!explicitlen && !hexconstant && !fake) asar_throw_warning(0, warning_id_implicitly_sized_immediate); if (len==1) { withlen(1); write1(byte); write1(num); } \
																					 else { withlen(2); write1((unsigned int)byte); write2(num); } return true; }
#define as_xy(  op, byte) if (is_65816(op)) { if(!explicitlen && !hexconstant && !fake) asar_throw_warning(0, warning_id_implicitly_sized_immediate); if (len==1) { withlen(1); write1(byte); write1(num); } \
																					 else { withlen(2); write1((unsigned int)byte); write2(num); } return true; }
#define as_rep( op, byte) if (is_65816(op)) { if (pass<2 && !fake) { num=getnum(par); } if(foundlabel) asar_throw_error(0, error_type_block, error_id_no_labels_here); withlen(0); for (unsigned int i=0;i<num;i++) { write1((unsigned int)byte); } recent_opcode_num = num; return true; }
#define as_rel1(op, byte) if (is_65816(op)) { int pos=(!foundlabel)?(int)num:(int)num-((snespos&0xFFFFFF)+2); withlen(1); write1((unsigned int)byte); write1((unsigned int)pos); \
													...
													return true; }
#define as_rel2(op, byte) if (is_65816(op)) { int pos=(!foundlabel)?(int)num:(int)num-((snespos&0xFFFFFF)+3); withlen(2); write1((unsigned int)byte); write2((unsigned int)pos);\
...
#define onlythe8(left, right, offset) else if (match_65816(left, right)) do { init_index(left, right); the8(offset, 1); end_65816(); } while(0)
	...
	else if (match_65816("(", ")") && confirmqpar(substr(word[1]+1, (int)(strlen(word[1]+1)-1))))
	{
		init('(', ')');
		the8(0x12, 1);
		as1("PEI", 0xD4);
	...
	{
		init_index("", ",x");
		if (match_65816("(", ")") && confirmqpar(substr(word[1] + 1, (int)(strlen(word[1] + 1) - 2 - 1)))&& !fake) asar_throw_warning(0, warning_id_65816_yy_x_does_not_exist);
		the8(0x15, 1);
		the8(0x1D, 2);
		the8(0x1F, 3);
		thenext8(0x16, 1);
	...
	else if (match_65816("", ",y"))
	{
		...
		as1("STX", 0x96);
		as2("LDX", 0xBE);
		if (len==1 && (is_65816("ORA") || is_65816("AND") || is_65816("EOR") || is_65816("ADC") || is_65816("STA") || is_65816("LDA") || is_65816("CMP") || is_65816("SBC")))
		{
	...
	{
		if ((is_65816("MVN") || is_65816("MVP")) && confirmqpar(par))
		{
			int numargs;
			...
			{
				withlen(2);
				write1(is_65816("MVN")?(unsigned int)0x54:(unsigned int)0x44);
